...
namespace softwareCertificate.DALL
{
    public class HelperData
    {
        ...
        public int EditSoftwareInfo(softwareInfo si)
        {
            try
            {
                ...
                if(newsoftware != null)
                {
                    ...
                    newsoftware.softCodeDefine = si.softCodeDefine;
        ...
        public int EditVahedUser(Int16 userCode,List<VahedUser> lv)
        {
            try
            {
                var v = from i in db.VahedUsers
                ...
                foreach (VahedUser vu in v)
                {
                    vu.userCode = userCode;
                    db.SubmitChanges();
                    db.VahedUsers.DeleteOnSubmit(vu);

                }
                foreach (VahedUser r in lv)
                {
                    r.userCode = userCode;
                    r.deleted = false;
                    db.VahedUsers.InsertOnSubmit(r);
        ...
        public string changePass(string oldPass, string newPass, Int16 userCode)
        {
            try
            {
        ...
        public string searchUserEmail(string user, string Email)
        {
            try
            {
                var usr = from i in db.userinfos
                          where i.userName == user && i.email == Email
                          select i;
                userinfo u = usr.FirstOrDefault();
                if (u != null)
                {
                    return u.userCode.ToString();
                }
                else
                    return "0";
            }
        ...
        public string changPass(string userName,string newPass)
        {
            try
            {
                var ur = from i in db.userinfos
