...
namespace softwareCertificate.BLL
{
    public class AddSoftwareBLL
    {
        ...
        public Results<DataTable> SearchReqSoftware(int firstRow)
        {
           //bool admin = Convert.ToBoolean(System.Web.HttpContext.Current.Session["admin"]);
            DataTable list = new DataTable();
            Results<DataTable> result = new Results<DataTable>();
            try
            {
                ...
                helperSearch sHelper = new helperSearch();
                list = sHelper.searchSoftware1(Convert.ToInt16(System.Web.HttpContext.Current.Session["userCode"]), firstRow, firstRow + 6-1);
        ...
        public object SofSearchInTable(string SoftwareName, string CompanyName, string VahedValue, string ZoneValue,string StartDate,string EndDate,int firstRow)
        {
             DataTable list = new DataTable();
            Results<DataTable> result = new Results<DataTable>();
            try
            {
                ...
                helperSearch sHelper = new helperSearch();
                list = sHelper.searchSoftware(firstRow, firstRow + 6 - 1,SoftwareName, CompanyName, vahedCode, zoneCode, StartDate, EndDate);
        ...
        public object updateSoftReq(softwareInfo rn)
        {
            ...
            try
            {
                ...
                if (rn.vahedCode != 0)
                {
                    rn.softCodeDefine = (rn.vahedCode).ToString() + (rn.zoneCode).ToString() + (rn.softwareCode).ToString();
                }
                else
                    rn.softCodeDefine = "0" + (rn.zoneCode).ToString() + (rn.softwareCode).ToString();
        ...
        public object UpgradeReqSeBySoft(short softwareCode)
        {
            DataTable list = new DataTable();
            Results<DataTable> result = new Results<DataTable>();
            try
            {
                ...
                helperSearch sHelper = new helperSearch();
                list = sHelper.searchUpgrade(softwareCode);
